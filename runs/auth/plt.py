...
# runs/auth/C1:bunsan/base/0.3/

# 以下の部分を変更して最も大きいTotal＿walk_timeにしたい
# LOG_PATTERN = re.compile(r"\[TOTAL\]\s*controller_duration_sum=([0-9.]+)s")
# 例: [TOTAL] Total_walk_time=12.34s
#     [TOTAL] total_walk_time = 12.34 s
#     [TOTAL] controller_duration_sum=12.34s なども拾えるように柔らかく
TARGET_KEY = "Total walk time (sum over all servers):"

LOG_PATTERN = re.compile(
    rf"{re.escape(TARGET_KEY)}\s*([0-9.]+)\s*s",
    re.IGNORECASE,
)
        ...
        with log_file.open() as f:
            for line in f:
                ...
                if m:
                    ...
                    if graph == "fb-caltech-connected-nocache":
                        continue
                    data.setdefault(graph, {})[exp_name] = time
                    print(f"[{exp_name}] {graph}: {time}s")
                    break
...
ax.legend()
ax.set_title("Controller Duration Comparison (base vs cache)(NG=0.3)")
