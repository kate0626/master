...
class NodeEdgeFixedPartitioner:
    """
    ★ ノードは全て server0、エッジは全て server1 に固定するパーティショナ。

    - node (int) -> 0
    - edge ("edge_u_v") -> 1

    注意:
      server_count は最低 2 必要。
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="modulo",
        choices=["modulo", "community", "metis", "node-edge-fixed"],
...
def main() -> None:
    ...
    if args.partitioner_type == "community":
        ...
        if args.metis_use_bipartite_edges:
            if not args.edges:
                raise SystemExit(
                    "--edges is required when metis-use-bipartite-edges is set"
                )
    ...
    elif args.partitioner_type == "node-edge-fixed":
        # ★ ここが今回追加
