...
# # ログファイルを読み込み
# log_file = "all_results.txt"

# # スクリプトごとのAverage length格納用
# data = {"baseRW.py": [], "before.py": [], "after.py": []}
...
#             match = re.search(r"Running (.+?) \(Trial", line)
#             if match:
#                 current_script = match.group(1)
#         # Average length の抽出
#         elif "Average length" in line:
#             match = re.search(r"Average length:\s*([\d.]+)", line)
#             if match and current_script:
#                 data[current_script].append(float(match.group(1)))
...
# plt.boxplot(
#     [data["baseRW.py"], data["before.py"], data["after.py"]],
