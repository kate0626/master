...
if [[ -n "${CACHE_POLICIES_OVERRIDE:-}" ]]; then
  ...
  CACHE_POLICIES=("ppr_demand")   # デフォルト: ppr_demand のみ
fi
...
run_one_policy() {
  ...
  # このポリシーぶんの出力を tee でログに残す（サブシェル内で stdout/stderr をリダイレクト）
  {
    ...
    local REMOTE_CMD_BASE="NO_BACKTRACK=${NO_BACKTRACK} python3 base/proposed_cache/split_remote_server_proposed.py \
