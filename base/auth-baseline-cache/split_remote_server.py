...
#   --server-id 0 \
#   --server-count 2 \
#   --edges dataset/Louvain/graph/fb-caltech-connected.gr \
#   --host 10.58.60.6 \
#   --port 3000 \
#   --server-endpoints 10.58.60.6:3000 10.58.60.11:3000 \
#   --node-to-starts-file base/auth-many-server/data/splits/fb-caltech-connected/0.3/node_to_starts_server0.json \
#   --owned-hints-only
...
#   --port 3000 \
#   --server-endpoints 10.58.60.6:3000 10.58.60.11:3000 \
#   --node-to-starts-file base/auth-many-server/data/splits/fb-caltech-connected/0.3/node_to_starts_server1.json \
#   --owned-hints-only
...
#     """ARC cache for authorization decisions.

#     T1/T2 store real cache entries, while B1/B2 are ghost lists storing only keys.
#     Capacity is the number of real cached entries.
#     """
...
#                 else max(1, len(self.b2) // max(1, len(self.b1)))
#             )
...
#                 else max(1, len(self.b1) // max(1, len(self.b2)))
#             )
...
#         self._ensure_space_for_new_entry(key)
#         self.t1[key] = value
...
#         if self.t1 and (
#             (incoming_key in self.b2 and len(self.t1) == self.p)
...
#     def _ensure_space_for_new_entry(self, incoming_key: Tuple[int, str]) -> None:
#         real_size = len(self.t1) + len(self.t2)
#         if real_size >= self.capacity:
#             self._replace(incoming_key)
#         else:
#             total_size = real_size + len(self.b1) + len(self.b2)
...
# def build_authz_cache(policy: str, capacity: int) -> BaseDecisionCache:
#     policy_name = (policy or "none").lower()
#     if policy_name == "none":
#         return NoDecisionCache()
#     if policy_name == "memo":
#         return UnlimitedDecisionCache()
#     if policy_name == "lru":
#         return LRUDecisionCache(capacity)
#     if policy_name == "arc":
#         return ARCDecisionCache(capacity)
...
#                 "auth_cache_policy": self.server.cache_policy,
#                 "auth_cache_capacity": self.server.cache_capacity,
...
#         s = self.server
#         policy = getattr(s, "cache_policy", "none")
#         capacity = getattr(s, "cache_capacity", 0)
#         s.authz_cache = build_authz_cache(policy, capacity)
#         s.auth_cache_hit = 0
#         s.auth_cache_miss = 0
...
#         self.walk_calls = 0
#         self.authz_cache = build_authz_cache(cache_policy, cache_capacity)
...
#         type=int,
#         default=0,
#         help="Maximum number of cached authorization decisions for LRU/ARC. Ignored for 'none'.",
#     )
#     parser.add_argument(
#         "--auth-file",
#         type=str,
