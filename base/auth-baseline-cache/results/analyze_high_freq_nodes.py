...
python3 base/auth-baseline-cache/analyze_cache_causes.py \
  --results-dir base/auth-baseline-cache/results/alpha0.01_walks_100_capa_100 --graphs amazon0601 \
...
# 高頻度ノード抽出
# ---------------------------------------------------------------------------
def extract_high_freq(
    run: dict,
    threshold: int,
    lru_hits_per_node: dict | None = None,
    lru_miss_per_node: dict | None = None,
) -> list[dict]:
    """c >= threshold のノードを返す。
...
def plot_high_freq_summary(
    ...
    ax.set_title(
        f"(1) 高頻度ノードの c 分布 (CCDF)  [n={len(rows_all)}]",
        fontsize=11,
        fontweight="bold",
    )
    ...
    if out_degs.max() > 0:
        ax.set_xscale("log")
    ax.set_title("(2) c vs out_degree (= 次数の代理)", fontsize=11, fontweight="bold")
    ...
    bp = ax.boxplot(bx, positions=keys, widths=0.6, patch_artist=True, showfliers=True)
    ...
    for k in keys:
        ax.text(
            k,
            ax.get_ylim()[1] * 0.92 if ax.get_ylim()[1] > 0 else max(cs),
            f"n={len(by_dist[k])}",
            ha="center",
            fontsize=8,
            color="#555",
    ...
    ax.set_yscale("log")
    ax.set_title(
        "(3) c vs start からの BFS 距離  ← BFS prefetch の効きどころ確認",
        fontsize=10,
        fontweight="bold",
    )
    ...
    for tgt, col in [(0.5, "#ff7f0e"), (0.8, "#d62728"), (0.95, "#9467bd")]:
        try:
            ...
            ax.axvline(
                ...
                linewidth=1.0,
                alpha=0.7,
                label=f"top-{n_needed} で {int(tgt*100)}% カバー",
            )
        except Exception:
            pass
    ...
    ax.set_title(
        "(4) 上位 N ノードで全 lookups の何 % をカバーするか",
        fontsize=11,
        fontweight="bold",
    )
    ax.grid(True, linestyle="--", alpha=0.4)
...
def main() -> None:
    ...
    for graph_dir in sorted(results_dir.iterdir()):
        ...
        print(
            f"  c       : min={c_arr.min()}  median={int(np.median(c_arr))}  "
            f"p95={int(np.percentile(c_arr,95))}  max={c_arr.max()}"
        )
        print(
            f"  dist    : min={d_arr[d_arr>=0].min() if (d_arr>=0).any() else 'n/a'}  "
            f"median={int(np.median(d_arr[d_arr>=0])) if (d_arr>=0).any() else 'n/a'}  "
            f"max={d_arr.max()}"
        )
        print(
            f"  out_deg : min={deg_arr.min()}  median={int(np.median(deg_arr))}  "
            f"max={deg_arr.max()}"
        )
        print(
            f"  auth_fail_rate : mean={fail_arr.mean():.4f}  max={fail_arr.max():.4f}"
        )
        ...
        for tgt in (0.5, 0.8, 0.95):
            ...
            print(
                f"  top-{n_need:>4} ノードで このセット内の {int(tgt*100)}% lookups をカバー"
            )
        ...
        if args.with_lru_sim:
            ...
            print(
                f"  is_lru_blind (sim で 1 度も hit せず) : {len(blind_rows)} / {len(rows_all)} "
                f"({100*len(blind_rows)/len(rows_all):.1f}%)"
            )
            print(
                f"  partial hit (0 < HR < 0.5)          : {len(partial_rows)} "
                f"({100*len(partial_rows)/len(rows_all):.1f}%)"
            )
            print(
                f"  well covered (HR >= 0.5)            : {len(covered_rows)} "
            ...
            if blind_rows:
                ...
                print(
                    f"    dist    : median={int(np.median(bd_arr[bd_arr>=0])) if (bd_arr>=0).any() else 'n/a'} "
                    f"max={bd_arr.max()}"
                )
                print(
                    f"    out_deg : median={int(np.median(bdeg))} max={int(bdeg.max())}"
                )
...
# LRU 取りこぼしパターン専用プロット
# ---------------------------------------------------------------------------
def plot_lru_miss_pattern(
    graph: str, rows_all: list[dict], cap: int, out_path: Path
) -> None:
    ...
    ax.set_ylim(-0.1, 1.05)
    ax.set_title(
        "(2) out_degree 別 LRU vs memo hit_rate", fontsize=11, fontweight="bold"
    )
    ax.legend(fontsize=9)
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)
    ...
    ax.set_ylim(-0.1, 1.05)
    ax.set_title(
        "(3) lookup 頻度別 LRU vs memo hit_rate", fontsize=11, fontweight="bold"
    )
    ax.legend(fontsize=9)
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)

    fig.suptitle(
        f"{graph} — LRU の取りこぼしパターン (cap={cap})",
        fontsize=12,
        fontweight="bold",
        y=1.01,
    )
