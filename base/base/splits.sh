...
REMOTE_CMD_BASE="python3 base/base/split_remote_server.py \\
...
cleanup() {
  ...
  for entry in "${SERVERS[@]}"; do
    eval "$entry"
    ssh -o ConnectTimeout=5 "$host" "pkill -f base/base.py || true" >/dev/null 2>&1 || true
  done
  echo ">>> [CLEANUP] 完了。"
}
...
PID=\$!
echo \$PID > split_remote_server.pid

# HTTPで起動待ち（/health が返るまで待つ）
timeout ${TIMEOUT}s bash <<INNER
set -e
...
EOF
  ...
  if [[ $? -ne 0 ]]; then
    ...
    # 失敗時にログの末尾を表示して原因が見えるようにする（任意）
    ssh "$host" "tail -n 80 split_remote_server.log || true"
  fi
}
